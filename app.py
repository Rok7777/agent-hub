...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 1: LOTI (obstoječa logika — nespremenjena)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 2: BLAGAJNA — DNEVNI IZKUPIČEK
# ══════════════════════════════════════════════════════════════════════════════
...
with main_tab2:
    ...
    # ── Info razlaga ──────────────────────────────────────────────────────────
    with st.expander("ℹ️ Kako deluje agent", expanded=False):
        st.markdown("""
        Agent za vsak osnutek temeljnice naredi naslednje:

        **1. Prebere knjižbe** iz osnutka
        - Konto `1652` = plačilo z kartico
        - Konto `1000` = plačilo z gotovino
        - Analitika določi blagajno (MPK1 / MPK2 / MPK3 / MPOC)

        **2. Kreira blagajniški dnevnik** (datum = datum osnutka)

        **3. Doda blagajniški prejemek**
        - Stranka: Končni kupec - maloprodaja
        - Tip: Dnevni iztržek
        - Znesek: kartica + gotovina skupaj

        **4. Doda blagajniški izdatek** (2 vrstici)
        - Polog gotovine - domača DE → znesek gotovine
        - Terjatev za plačila z kartico → znesek kartice

        **5. Popravi temeljnico**
        - Konto 1652 → 120000 (Končni kupec - maloprodaja) z vsoto
        - Konto 1000 → izbriše
        """)

    # ── Pregled blagajn ───────────────────────────────────────────────────────
    st.subheader("Blagajne")
    blagajne_info = {
    ...
    df_blagajne = pd.DataFrame([
        {"Šifra": k, "Naziv": v["naziv"], "bg_id": v["bg_id"]}
        for k, v in blagajne_info.items()
    ])
    st.dataframe(df_blagajne, use_container_width=True, hide_index=True)

    st.divider()

    # ── Predpogoji ────────────────────────────────────────────────────────────
    st.subheader("Predpogoji")
    col_a, col_b = st.columns(2)
    with col_a:
        st.info(
            "**Chrome mora biti odprt** z remote debugging:\n\n"
            "```\nchrome.exe --remote-debugging-port=9222\n```\n\n"
            "In prijavljen v Minimax."
        )
    with col_b:
        st.info(
            "Agent bo obdelal **vse osnutke** ki so trenutno v temeljnicah.\n\n"
            "Priporočljivo: najprej preveri osnutke ročno preden zaženeš agenta."
        )
    ...
    with col_run:
        run_blagajna = st.button(
            "▶️ Zaženi agent blagajne",
            type="primary",
            use_container_width=True,
            key="run_blagajna_agent",
        )

    if run_blagajna:
        st.session_state["blagajna_running"] = True
        st.session_state["blagajna_log"] = []

    if st.session_state.get("blagajna_running"):
        st.subheader("📋 Log izvajanja")
        log_placeholder = st.empty()
        status_placeholder = st.empty()

        with status_placeholder:
            st.info("⏳ Agent se izvaja … ne zapiraj okna.")

        log_lines = []

        try:
            process = subprocess.Popen(
                [sys.executable, "agent_blagajna.py"],
            ...
            st.session_state["blagajna_running"] = False

            if process.returncode == 0:
                status_placeholder.success("✅ Agent uspešno zaključil!")
            else:
                status_placeholder.error(f"❌ Agent se je končal z napako (koda {process.returncode})")

        except FileNotFoundError:
            status_placeholder.error(
                "❌ Datoteka `agent_blagajna.py` ni najdena. "
                "Prepričajte se da je v isti mapi kot `app.py`."
            )
            st.session_state["blagajna_running"] = False

        except Exception as e:
            status_placeholder.error(f"❌ Napaka pri zagonu agenta: {e}")
            st.session_state["blagajna_running"] = False

    # ── Prikaz zadnjega loga (po zaključku) ───────────────────────────────────
    elif st.session_state.get("blagajna_log"):
        log_lines = st.session_state["blagajna_log"]
        st.subheader("📋 Log zadnjega zagona")
        st.code("\n".join(log_lines), language="text")

        # Izlušči povzetek iz loga
        obdelani = [l for l in log_lines if l.strip().startswith("✓")]
        ...
        if obdelani or napake:
            ...
            with col_ok:
                st.metric("✅ Obdelano", len(obdelani))
            with col_err:
                st.metric("❌ Napake", len(napake))

        if st.button("🗑 Počisti log", key="clear_blagajna_log"):
            st.session_state["blagajna_log"] = []
            st.rerun()
...
# ── Noga ──────────────────────────────────────────────────────────────────────
st.divider()
st.caption("Agent Hub v1.1 · Loti (Minimax API) + Blagajna (Playwright)")
